...
void Interpreter::registerBuiltins() {
    globals["PROJECT_NAME"] = Value("Prosto+");
    globals["VERSION"] = Value("1.0.0");
    globals["system"] = Value(platformName());
    globals["now"] = Value(formatDate("%Y-%m-%d %H:%M:%S"));

    builtins["true"] = Value(true);
    builtins["True"] = Value(true);
    builtins["false"] = Value(false);
    builtins["False"] = Value(false);
    builtins["None"] = Value();
    builtins["null"] = Value();

    builtins["print"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["input"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["exit"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        exit(0);
        return Value();
    }, "exit");

    builtins["abs"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0LL);
        if (args[0].type == VT::Int) return Value(llabs(args[0].i));
        return Value(fabs(args[0].toFloat()));
    }, "abs");

    builtins["len"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0LL);
        Value& v = args[0];
    ...
    builtins["min"] = Value::makeNativeFunction([seqOrArgs](Interpreter&, vector<Value>& args) {
    ...
    builtins["max"] = Value::makeNativeFunction([seqOrArgs](Interpreter&, vector<Value>& args) {
        auto vals = seqOrArgs(args);
    ...
    builtins["sum"] = Value::makeNativeFunction([seqOrArgs](Interpreter&, vector<Value>& args) {
        auto vals = seqOrArgs(args);
        bool allInt = true;
        long long si = 0;
        double sf = 0.0;
    ...
    builtins["str"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(args[0].toStr());
    }, "str");

    builtins["int"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0LL);
    ...
    builtins["float"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0.0);
        return Value(args[0].toFloat());
    }, "float");

    builtins["round"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0LL);
    ...
    builtins["upper"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(upperString(args[0].toStr()));
    }, "upper");

    builtins["lower"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(lowerString(args[0].toStr()));
    }, "lower");

    builtins["split"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["join"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["replace"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["find"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value(-1LL);
        auto pos = args[0].toStr().find(args[1].toStr());
        return Value(pos == string::npos ? -1LL : (long long)pos);
    }, "find");

    builtins["slice"] = Value::makeNativeFunction([](Interpreter& in, vector<Value>& args) {
    ...
    builtins["time"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        return Value((long long)chrono::duration_cast<chrono::seconds>(
            chrono::system_clock::now().time_since_epoch()).count());
    }, "time");

    builtins["date"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        string fmt = args.empty() ? "%Y-%m-%d %H:%M:%S" : args[0].toStr();
        return Value(formatDate(fmt));
    }, "date");

    builtins["basename"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(fs::path(args[0].toStr()).filename().string());
    }, "basename");

    builtins["dirname"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(fs::path(args[0].toStr()).parent_path().string());
    }, "dirname");

    builtins["extname"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(fs::path(args[0].toStr()).extension().string());
    }, "extname");

    builtins["joinpath"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value("");
        return Value((fs::path(args[0].toStr()) / args[1].toStr()).string());
    }, "joinpath");

    builtins["type"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["sound"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty() || args[0].isNull()) {
            cout << '\a';
            cout.flush();
        } else {
            openWithSystem(args[0].toStr());
        }
        return Value();
    }, "sound");

    builtins["zip_folder"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["list"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return makeListFromVector({});
    ...
    builtins["dict"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (!args.empty() && args[0].isDict()) return args[0];
        return Value::makeDict(make_shared<Dict>());
    }, "dict");

    builtins["tuple"] = builtins["list"];
    builtins["set"] = builtins["list"];

    builtins["range"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["sorted"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["enumerate"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        vector<Value> out;
    ...
    builtins["zip"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        vector<Value> out;
    ...
    builtins["map"] = Value::makeNativeFunction([](Interpreter& in, vector<Value>& args) {
    ...
    builtins["filter"] = Value::makeNativeFunction([](Interpreter& in, vector<Value>& args) {
        if (args.size() < 2 || !args[0].isFunction() || !args[1].isList()) return makeListFromVector({});
        vector<Value> out;
        for (auto& x : *args[1].list) {
            vector<Value> callArgs{x};
    ...
    builtins["format"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
    ...
    builtins["template"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
    ...
    builtins["pi"] = Value(M_PI);
    builtins["e"] = Value(M_E);

    builtins["sin"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        return Value(sin(args.empty() ? 0.0 : args[0].toFloat()));
    }, "sin");

    builtins["cos"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        return Value(cos(args.empty() ? 0.0 : args[0].toFloat()));
    }, "cos");

    builtins["tan"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        return Value(tan(args.empty() ? 0.0 : args[0].toFloat()));
    }, "tan");

    builtins["sqrt"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        return Value(sqrt(args.empty() ? 0.0 : args[0].toFloat()));
    }, "sqrt");

    builtins["log"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0.0);
    ...
    builtins["floor"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        return Value((long long)floor(args.empty() ? 0.0 : args[0].toFloat()));
    }, "floor");

    builtins["ceil"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        return Value((long long)ceil(args.empty() ? 0.0 : args[0].toFloat()));
    }, "ceil");

    builtins["pow"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value(0LL);
        return applyBinary("**", args[0], args[1]);
    }, "pow");

    builtins["json_parse"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value();
        try {
    ...
    builtins["json_stringify"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("null");
    ...
    builtins["http_get"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["http_post"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["http_put"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["http_delete"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["http_download"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value(false);
        int timeout = args.size() > 2 ? (int)args[2].toInt() : 30;
        return Value(httpDownload(args[0].toStr(), args[1].toStr(), timeout));
    }, "http_download");

    builtins["HttpResponse"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        return Value::makeNativeObject(make_shared<HttpResponseNative>(0, vector<pair<string, string>>{}, ""));
    }, "HttpResponse");

    builtins["RE_IGNORECASE"] = Value(2LL);
    builtins["RE_MULTILINE"] = Value(8LL);
    builtins["RE_DOTALL"] = Value(16LL);
    ...
    builtins["regex_match"] = Value::makeNativeFunction([makeRegex](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value(false);
        int flags = args.size() > 2 ? (int)args[2].toInt() : 0;
        return Value(regex_match(args[1].toStr(), makeRegex(args[0].toStr(), flags)));
    }, "regex_match");

    builtins["regex_fullmatch"] = builtins["regex_match"];

    builtins["regex_search"] = Value::makeNativeFunction([makeRegex](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value();
        int flags = args.size() > 2 ? (int)args[2].toInt() : 0;
        smatch m;
    ...
    builtins["regex_findall"] = Value::makeNativeFunction([makeRegex](Interpreter&, vector<Value>& args) {
    ...
    builtins["regex_replace"] = Value::makeNativeFunction([makeRegex](Interpreter&, vector<Value>& args) {
    ...
    builtins["regex_split"] = Value::makeNativeFunction([makeRegex](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return makeListFromVector({});
        int flags = args.size() > 2 ? (int)args[2].toInt() : 0;
        string s = args[1].toStr();
        auto re = makeRegex(args[0].toStr(), flags);
    ...
    builtins["regex_escape"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        string s = args[0].toStr();
        string out;
        for (char c : s) {
    ...
    builtins["base64_encode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        string s = args[0].toStr();
        return Value(base64EncodeImpl((const unsigned char*)s.data(), s.size(), false));
    }, "base64_encode");

    builtins["base64_decode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(base64DecodeImpl(args[0].toStr(), false));
    }, "base64_decode");

    builtins["base64url_encode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        string s = args[0].toStr();
        return Value(base64EncodeImpl((const unsigned char*)s.data(), s.size(), true));
    }, "base64url_encode");

    builtins["base64url_decode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(base64DecodeImpl(args[0].toStr(), true));
    }, "base64url_decode");

    builtins["hex_encode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(hexEncodeString(args[0].toStr()));
    }, "hex_encode");

    builtins["hex_decode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(hexDecodeString(args[0].toStr()));
    }, "hex_decode");

    builtins["url_encode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(urlEncodeString(args[0].toStr()));
    }, "url_encode");

    builtins["url_decode"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(urlDecodeString(args[0].toStr()));
    }, "url_decode");

    builtins["html_escape"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(htmlEscapeString(args[0].toStr()));
    }, "html_escape");

    builtins["html_unescape"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(htmlUnescapeString(args[0].toStr()));
    }, "html_unescape");

    builtins["utf8_bytes"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        vector<Value> out;
        if (!args.empty()) {
            string s = args[0].toStr();
    ...
    builtins["utf8_str"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        string out;
        if (!args.empty() && args[0].isList()) {
    ...
    builtins["md5"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(evpDigestHex("md5", (const unsigned char*)args[0].toStr().data(), args[0].toStr().size()));
    }, "md5");

    builtins["sha1"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(evpDigestHex("sha1", (const unsigned char*)args[0].toStr().data(), args[0].toStr().size()));
    }, "sha1");

    builtins["sha256"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(evpDigestHex("sha256", (const unsigned char*)args[0].toStr().data(), args[0].toStr().size()));
    }, "sha256");

    builtins["sha512"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(evpDigestHex("sha512", (const unsigned char*)args[0].toStr().data(), args[0].toStr().size()));
    }, "sha512");

    builtins["crc32"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0LL);
        return Value((long long)crc32String(args[0].toStr()));
    }, "crc32");

    builtins["hmac_sha256"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value("");
        return Value(hmacSha256Hex(args[0].toStr(), args[1].toStr()));
    }, "hmac_sha256");

    builtins["hash_file"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        string algo = args.size() > 1 ? args[1].toStr() : "sha256";
        return Value(evpFileDigestHex(args[0].toStr(), algo));
    }, "hash_file");

    builtins["random"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        uniform_real_distribution<double> dist(0.0, 1.0);
        return Value(dist(RNG));
    }, "random");

    builtins["randint"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value(0LL);
        uniform_int_distribution<long long> dist(args[0].toInt(), args[1].toInt());
        return Value(dist(RNG));
    }, "randint");

    builtins["randfloat"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value(0.0);
        uniform_real_distribution<double> dist(args[0].toFloat(), args[1].toFloat());
        return Value(dist(RNG));
    }, "randfloat");

    builtins["choice"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty() || !args[0].isList() || args[0].list->empty()) return Value();
        uniform_int_distribution<size_t> dist(0, args[0].list->size() - 1);
        return (*args[0].list)[dist(RNG)];
    }, "choice");

    builtins["sample"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty() || !args[0].isList()) return makeListFromVector({});
    ...
    builtins["shuffle"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (!args.empty() && args[0].isList()) {
            shuffle(args[0].list->begin(), args[0].list->end(), RNG);
            return args[0];
        }
        return makeListFromVector({});
    }, "shuffle");

    builtins["seed"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (!args.empty()) RNG.seed((uint64_t)args[0].toInt());
        return Value();
    }, "seed");

    builtins["randbytes"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        long long n = args.empty() ? 0 : args[0].toInt();
        uniform_int_distribution<int> dist(0, 255);
        string out;
        for (long long i = 0; i < n; i++) out += (char)dist(RNG);
        return Value(hexEncodeString(out));
    }, "randbytes");

    builtins["uuid"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        return Value(generateUUID());
    }, "uuid");

    builtins["db_open"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["db_exec"] = Value::makeNativeFunction([getDb, bindParams](Interpreter&, vector<Value>& args) {
    ...
    builtins["db_query"] = Value::makeNativeFunction([getDb, bindParams](Interpreter&, vector<Value>& args) {
    ...
    builtins["db_close"] = Value::makeNativeFunction([getDb](Interpreter&, vector<Value>& args) {
        if (!args.empty()) {
            auto db = getDb(args[0]);
    ...
    builtins["mkdir"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (!args.empty()) fs::create_directories(args[0].toStr());
        return Value();
    }, "mkdir");

    builtins["rmdir"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (!args.empty()) {
            error_code ec;
    ...
    builtins["isfile"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(false);
        return Value(fs::is_regular_file(args[0].toStr()));
    }, "isfile");

    builtins["isdir"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(false);
        return Value(fs::is_directory(args[0].toStr()));
    }, "isdir");

    builtins["exists"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(false);
        return Value(fs::exists(args[0].toStr()));
    }, "exists");

    builtins["abspath"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
        return Value(fs::absolute(args[0].toStr()).string());
    }, "abspath");

    builtins["listdir"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["glob"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        vector<Value> out;
        if (!args.empty()) {
    ...
    builtins["filesize"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value(0LL);
        error_code ec;
    ...
    builtins["copyfile"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["movefile"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) throw ProstoError{"TypeError", "movefile(src,dst)"};
        fs::rename(args[0].toStr(), args[1].toStr());
        return Value();
    }, "movefile");

    builtins["search"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return makeListFromVector({});
        Value recursive = args.size() > 1 ? args[1] : Value(false);
        return searchFiles(args[0].toStr(), recursive);
    }, "search");

    builtins["EFCObject"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) throw ProstoError{"TypeError", "EFCObject(path) requires path"};
        return Value::makeNativeObject(make_shared<EFCObject>(args[0].toStr()));
    }, "EFCObject");

    builtins["exec_cmd"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["env_get"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
    ...
    builtins["env_set"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (args.size() < 2) return Value();
#ifdef _WIN32
    ...
    builtins["env_all"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        auto d = make_shared<Dict>();
        extern char** environ;
    ...
    builtins["sleep"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        double sec = args.empty() ? 0.0 : args[0].toFloat();
        this_thread::sleep_for(chrono::duration<double>(sec));
        return Value();
    }, "sleep");

    builtins["cwd"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        return Value(fs::current_path().string());
    }, "cwd");

    builtins["chdir"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (!args.empty()) fs::current_path(args[0].toStr());
        return Value();
    }, "chdir");

    builtins["tui_clear"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        cout << "\033[2J\033[H";
        cout.flush();
        return Value();
    }, "tui_clear");
    ...
    builtins["tui_color"] = Value::makeNativeFunction([ansiColor, ansiBg](Interpreter&, vector<Value>& args) {
        if (args.empty()) return Value("");
    ...
    builtins["tui_box"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        ...
        string top;
        if (!title.empty()) {
            top = "┌ " + title + " " + repeat("─", (size_t)(width - title.size() - 4)) + "┐";
        } else {
            top = "┌" + repeat("─", (size_t)(width - 2)) + "┐";
        }
        string bottom = "└" + repeat("─", (size_t)(width - 2)) + "┘";

        string out = top + "\n";
        for (auto& line : lines) {
            long long pad = max<long long>(0, width - (long long)line.size() - 4);
    ...
    builtins["tui_table"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        ...
        for (size_t i = 0; i < headers.size(); i++) {
            top += repeat("─", (size_t)colWidth) + (i + 1 == headers.size() ? "┐" : "┬");
            sep += repeat("─", (size_t)colWidth) + (i + 1 == headers.size() ? "┤" : "┼");
            bot += repeat("─", (size_t)colWidth) + (i + 1 == headers.size() ? "┘" : "┴");
        }
    ...
    builtins["tui_progress"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        ...
        string bar = repeat("█", (size_t)filled) + repeat("░", (size_t)(width - filled));
        ostringstream oss;
        oss << label << " [" << bar << "] " << (int)(pct * 100) << "%";
        return Value(oss.str());
    }, "tui_progress");

    builtins["tui_bar"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["tui_menu"] = Value::makeNativeFunction([](Interpreter& in, vector<Value>& args) {
        string title = args.size() > 0 ? args[0].toStr() : "";
        vector<Value> opts;
        ...
        Value box = in.builtins["tui_box"].func->nativeFn(in, boxArgs);
        cout << box.toStr() << endl;
    ...
    builtins["tui_confirm"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["tui_cursor"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
    ...
    builtins["tui_hide_cursor"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        cout << "\033[?25l";
        cout.flush();
        return Value();
    }, "tui_hide_cursor");

    builtins["tui_show_cursor"] = Value::makeNativeFunction([](Interpreter&, vector<Value>&) {
        cout << "\033[?25h";
        cout.flush();
        return Value();
    }, "tui_show_cursor");

    builtins["tui_title"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
        if (!args.empty()) cout << "\033]0;" << args[0].toStr() << "\007";
        cout.flush();
        return Value();
    }, "tui_title");

    builtins["tui_spinner"] = Value::makeNativeFunction([](Interpreter&, vector<Value>& args) {
